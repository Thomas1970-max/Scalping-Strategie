    ...
    public sealed class ReversalBouncePatternEvaluatorV2 : IPatternEvaluator
    {
            ...
            ZoneTracker tracker)
        {
            ...
            if (dir == OrderDirections.Buy)
            {
                ...
                if (!intentOk)
                    confirmPts = 0m;
                items.Add(new ScoreItem { Key = "Bestätigung", Points = confirmPts, TextDe = intentOk ? $"Bestätigung (Long): Close {awayTicks} Ticks über Zone -> +{confirmPts:0.0}" : $"Bestätigung (Long): Close über Zone, aber Verkaufsdruck (POC? {curr.PocDelta:+0;-0;0}) -> +0" });
            }
            else
            {
                ...
                if (!intentOk)
                    confirmPts = 0m;
                items.Add(new ScoreItem { Key = "Bestätigung", Points = confirmPts, TextDe = intentOk ? $"Bestätigung (Short): Close {awayTicks} Ticks unter Zone -> +{confirmPts:0.0}" : $"Bestätigung (Short): Close unter Zone, aber Kaufdruck (POC? {curr.PocDelta:+0;-0;0}) -> +0" });
            }
            score += confirmPts;
